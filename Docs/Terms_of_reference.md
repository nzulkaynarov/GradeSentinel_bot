...
Families (Семьи): Группирующая сущность. К ней привязана подписка.

Parents (Родители): id, fio, phone, telegram_id (заполняется при входе).

Students (Ученики): id, fio, spreadsheet_id (ID таблицы дневника).

Family_Members: Таблица связей (кто из родителей и детей входит в какую семью).
...
Админ (ты) через специальную команду или конфиг добавляет Семью.

Админ добавляет Родителей (ФИО и телефон).

Админ добавляет Детей (ФИО и ID таблицы).
